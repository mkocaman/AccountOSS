...
public class GetPurchaseReportQueryHandler : IRequestHandler<GetPurchaseReportQuery, Result<PurchaseReportDto>>
{
    ...
    public async Task<Result<PurchaseReportDto>> Handle(GetPurchaseReportQuery request, CancellationToken cancellationToken)
    {
        if (_currentUser.CompanyId == null)
            return Result<PurchaseReportDto>.Fail("Şirket bilgisi bulunamadı");
        ...
        if (request.SupplierId.HasValue)
            query = query.Where(i => i.CustomerId == request.SupplierId.Value);
        ...
        // Günlük alışlar
        var dailyPurchases = filteredInvoices
        ...
        // Aylık alışlar
        var monthlyPurchases = filteredInvoices
        ...
        // Tedarikçilere göre alışlar
        var purchaseBySupplier = filteredInvoices
            .GroupBy(i => new { i.CustomerId, i.Customer.Code, i.Customer.Name })
            .Select(g => new PurchaseBySupplierDto
            {
                SupplierId = g.Key.CustomerId,
        ...
        var purchaseByProduct = filteredInvoices
        ...
        return Result<PurchaseReportDto>.Ok(report);
    }
}
