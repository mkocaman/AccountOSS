...
import type { CreatePurchaseOrderRequest, CreatePurchaseOrderItemRequest } from '@/types/purchaseOrder';
import { formatCurrency } from '@/lib/utils';
import { usePageTitle } from '@/hooks/usePageTitle';

interface FormItem extends CreatePurchaseOrderItemRequest {
  key: string;
  product?: any;
}
...
export default function PurchaseOrderForm() {
  ...
  usePageTitle(id ? 'Sipariş Düzenle' : 'Yeni Sipariş');
  ...
  // Fetch suppliers (Partners with type Supplier or Both)
  ...
  const saveMutation = useMutation({
    mutationFn: (data: CreatePurchaseOrderRequest) => {
      if (id) {
        return purchaseOrdersApi.update(id, data);
      }
      return purchaseOrdersApi.create(data);
    },
    onSuccess: () => {
  ...
  const handleSupplierChange = (supplierId: string) => {
    const supplier = suppliers?.items?.find((s: any) => s.id === supplierId);
    if (supplier) {
      setSelectedCurrency(supplier.currency);
  ...
  const handleProductSelect = (key: string, productId: string) => {
    ...
    if (product) {
      ...
      updateItem(key, 'unitPrice', product.purchasePrice || 0);
    }
  };
    ...
    <div className="p-6">
      ...
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900">
          {id ? 'Satın Alma Siparişi Düzenle' : 'Yeni Satın Alma Siparişi'}
        </h1>
      </div>
